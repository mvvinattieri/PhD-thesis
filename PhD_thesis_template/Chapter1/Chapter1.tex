...
Therefore, after all of the analyses, we adopt the mean as the best posterior frailty risk summary and thus we perform final computations only with the Multivariate frailty Cox model.

Prediction accuracy for the posterior mean and the family history is reported in Table \ref{tab:1.3} through the Harrell's Concordance index, respectively for the Multivariate frailty Cox model and the two Univariate Cox models, distinguishing according to the nature of the family history. Also, the accuracy of the binary classification can be analyzed by reporting the AUC, the positive predicted value PPV, and the negative predictive value (NPV). As a result all measures improve as the family size increases, as expected. This can be appreciated also from the ROC curve in Figure \ref{fig:1.4}, where the scenario with maximum family size equal to twenty correctly classifies the 97\% of families in the right group, the 15\% more than the scenario with maximum five family members. However, all the results are promising since the AUC is in the range 0.7-0.85: this indicates that around the 70\%-85\% of families are correctly classified in the right group also with small family sizes. Now, the negative predictive value (NPV) is always significantly higher than the positive predictive value, meaning that to the higher value of the AUC the NPV contributes more. 
