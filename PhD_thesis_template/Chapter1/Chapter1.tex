...
Recall from above the general Lehmann family of distributions that defines a proportional hazards (PH) structure:
\begin{equation}
\left\{  S_{\alpha}(t) = \left[ \widetilde{S}_0(t) \right]^{\alpha}, \ \ \alpha >0  \right\},   
\label{formula:1.PH}
\end{equation}
where $\widetilde{S}(t)$ is the traditional baseline survival function and the term $\alpha$ modifies it to become the (also traditional) survival function $S_\alpha(t)$. It is then easy to check that, with obvious notation, $\lambda_{\alpha}(t) = \alpha \lambda_0(t)$ for any choice of positive and finite $\alpha$. When $\alpha$ is modelled through a regression structure one has the celebrated Cox proportional hazards survival model \cite{cox1972regression}.

We extend the general Lehmann family in Equation \ref{formula:1.PH} to the model defined by the more general Lehmann Cure-Rate family obtained by applying the Lehmann power transformation to a cure-rate survival function:
\begin{equation}
\left\{  S_{\alpha}(t) = \left[ p + (1-p) \widetilde{S}(t) \right]^{\alpha}, \ \ \alpha >0  \right\}.
\label{formula:1.LehmannCR}
\end{equation}

For a fixed value $\alpha$, the model in \ref{formula:1.LehmannCR} is such that the survival function $S_{\alpha}(t)$ also defines a cure-rate model. It is easy to check that $\lim_{t \rightarrow \infty} S_{\alpha}(t) = p^{\alpha}$, and that $S_{\alpha}(t)$ can be written as
\[
...
The term $\alpha$ may depend on a latent or observed covariate $R$, so that $\alpha = \alpha(R)$, that in our case it can be seen as the family-specific frailty risk.
...
\subsection{Data generating process from the Lehmann Cure-Rate model with multiplicative gamma frailty}
