\chapter{An Application of risk prediction models to the Swedish Multi-Generational Breast Cancer Registry}{Joint work with Marco Bonetti\footnote{Carlo F. Dondena Research Center, Bocconi Institute of Data Science and Analytics, Department of Social and Political Sciences, Bocconi University, Milan, 20136, Italy, email: marco.bonetti@unibocconi.it.}, and Kamila Czene\footnote{Department of Medical Epidemiology and Biostatistics, Karolinska Institutet, Solna, 171 77, Sweden, email: kamila.czene@ki.se.}}
...
\begin{abstract}
We focus on a specific aspect related to the disease development: the study of family-specific risk. Although this issue can be applied to various diseases, our specific focus is on breast cancer. We assume that families are characterized by a true family-specific risk of developing breast cancer that is latent and unchanged from birth. To study this phenomenon, we have developed a parametric cure rate frailty multivariate survival model where subjects contribute as part of a family, which is seen as a cluster. This model allows for a cure rate, so that only a fraction of the population will experience the breast cancer onset.

Furthermore, we have explored models that include an observable binary risk indicator that can be used as a ``guess'' for the true, latent risk. This indicator is the observed subject-specific family history of breast cancer, which is a function of the collection of breast cancer onset experiences in the family. We have compared these models and their semiparametric counterpart through simulations, as well as through their application to the Multi-Generational Swedish Breast Cancer Registry, with respect to parameter estimation and accuracy in risk prediction measured by the area under the ROC curve (AUC), for the binary splitting, and the Harrell's Concordance index.

Our findings demonstrate that the multivariate model, as expected, has the best performance in risk prediction, highlighting the crucial contribution of jointly model all the complete family information when targeting the highest-risk families for appropriate screening and prevention strategies. In contrast, the traditional use of a family history indicator is a much weaker tool.

\textbf{keywords}: breast cancer, family history, frailty models, risk prediction
\end{abstract}
%
\section{Introduction}\label{sec1}
Breast cancer (BC) risk prediction models have been extensively developed, with family history being just one of several risk factors. Other strong risk factors include genetic factors such as BRCA1, BRCA2, and TP53 mutations, single nucleotide polymorphisms (SNPs), clinical tests such as mammographic breast cancer density (MD) (see e.g. \cite{taroni2013optical}), and demographic features such as body mass index (BMI). These factors have been used alone or in combination to improve the predictive power of breast cancer risk models \cite{gail1989projecting, tyrer2004breast, rosner2008risk, antoniou2002comprehensive, antoniou2004boadicea, antoniou2008boadicea, thomas2004statistical}.

Gail \cite{gail1989projecting} developed a logistic regression for relative risk, incorporating the number of first-degree relatives who experienced breast cancer and other risk factors as covariates to compute the long-term probability of developing breast cancer. Tyrer and Cuzick \cite{tyrer2004breast} contributed to risk prediction models by combining genetic and familial risk factors on aggregated counting data instead of individual data. Rosner and Colditz \cite{rosner2008risk} constructed a logistic model for incidence affected by reproductive risk factors such as age at menarche, age at menopause, and age at childbirth. The BOADICEA model (\cite{antoniou2002comprehensive},\cite{antoniou2004boadicea}, \cite{antoniou2008boadicea}, \cite{thomas2004statistical}) is a strong alternative to these models, where joint multiplicative effects of genetic components explain breast cancer onset. The model infers a genetic latent quantity, the polygenic component, that refers to the contribution of several genetics mutations with a singular small effect, on a generic disease development. The aim is to estimate the familial risk of developing breast cancer for predicting cancer risk according to the family history of the disease and other risk factors. The BOADICEA model is based on a subject-specific hazard, within a univariate framework, and it evaluates likelihood through a complex segregation analysis implemented in the Mendel software \cite{lee2019boadicea}. In contrast, in our models the family frailty will have an unspecified nature, and it will be studied within a multivariate framework. 

In this project, we propose a new model for predicting the risk of breast cancer onset based on the use of multivariate cure rate survival and frailty models. Multivariate frailty models are commonly used to model the dependence among time-to-events within the same subject, or across clusters \cite{chi2006joint, mazroui2013multivariate}.  The peculiarity in our project is that we consider families as our clusters, within which individuals share the same risk or frailty, so that the familiar time-to-events are jointly model under a conditional independence assumption given the frailty risk. This contribution is completely new as usually complete familiar information are rarely available into the dataset.

Specifically, we have constructed a parametric cure rate multivariate shared frailty model aimed at modeling the time to BC onset and ultimately deducing the hereditary susceptibility to BC. We have conducted a comparative analysis between this model and a multivariate shared frailty Cox model to evaluate the benefits of utilizing a parametric approach. Additionally, we conducted an internal analysis comparing the aforementioned parametric cure rate multivariate shared frailty model with other models that are expected to be weaker. These models include a parametric cure rate univariate frailty model that does not incorporate any covariates. Furthermore, we develop additional models that incorporate the first-degree family history of breast cancer as a covariate (treated as baseline), which can be measured either at the end of the follow-up period or in a time-varying manner. The family history is defined as the collection of breast cancer occurrences within a family, and is assigned a value of one if at least one family member has experienced the onset of breast cancer, or zero if none have. The three models we examine are a parametric constant family history model, a semiparametric constant family history model, and a semiparametric time-varying family history model. Notice that non of this involve the frailty, since this last is replaced with the family history indicator. We expect such family history indicator to be a weak indicator of breast cancer risk, although it still valuable as a protective factor for breast cancer screening \cite{lee2017differences}. Involving the family history into risk prediction models is usually done in the literature but we would like to assess that alternative solutions can easily increase, with outstanding outputs, the predictive power of breast cancer models. One of our goals is to confirm and quantify the added value of considering the whole family history experience in comparison of such indicator. Indeed, we expect the multivariate model to increase not only the precision in inference, but also this should translate into a more accurate risk prediction. Also, the cure rate structure allow us to explain and quantify the magnitude of this phenomenon, since it perfectly recovers the tail behaviour observed in the available data, and allow for the direct estimation of the cured fraction. 

We also explore the univariate version of the multivariate shared frailty Cox model and determine that it is only identifiable when incorporating familiar covariates, due to the lack of specification of the baseline hazard function. Consequently, we refrain from reporting any erroneous analysis on this model. Therefore, it is crucial for new patients to provide familial breast cancer information to obtain a reliable estimate of the probability of belonging to one of the highest-risk families. Indeed, the final goal of such risk-prediction efforts is to identify high-risk families for targeted medical intervention through more intensive screening surveillance and prevention strategies. Section \ref{sec2} provides an overview of the background of the setting. In Section \ref{sec3}, we describe the specific models employed in the analysis, and in Section \ref{sec4} we focus on the procedure of predicting the posterior risk. The simulation results are presented in Section \ref{sec5}, while the findings from the analysis of the Multi-Generational Swedish Breast Cancer registry are discussed in Section \ref{sec:1.6}. 

\section{Background}\label{sec2}
Our focus lies on the modelling of the age (in years) $T$ of breast cancer onset, where the conditional hazard function is denoted by $\lambda(t\mid r)$, where $R = r$ indicates the frailty quantity.  

The univariate multiplicative frailty model \cite{duchateau2008frailty} allows the hazard function to depend on the frailty quantity $R$ to capture the unobserved heterogeneity among subjects. The conditional hazard function at time $t$ is given by \[\lambda(t\mid r) = \alpha(r)\lambda_0(t),\] where $\alpha(r)$ can be any continuous function of the frailty, and $\lambda_0(t)$ indicates a ``baseline'' hazard function corresponding to a value of the frailty risk $r$ to be such that $\alpha(r)=1$. 

In presence of a vector of subject-specific covariates $x$, the frailty quantity explains the unobserved heterogeneity that the covariates are not able to capture. The conditional hazard function at time $t$ can be modelled as \[\lambda_x(t\mid R = r) =\alpha(r)\lambda_0(t;x),\] 

In the multivariate survival setting, the shared frailty model allows the hazard function to include a frailty that is defined as a family-specific quantity, so that families are clusters within which subjects share the same frailty risk. The conditional hazard function at time $t$ for subject $j$ of family $i$ can be defined as \[ \lambda_{x_{ij}}(t\mid r_i) = \alpha(r_i)\lambda_0(t;x_{ij}), \] with $x_{ij}$ the subject-specific vector of covariates and $r_i$ the frailty of family $i$.

The function $\alpha(r_i)$ can take several forms: one can use a polynomial form, an exponential form, or other (non-negative) forms. One may implement a hazard function $\lambda_{ij}(t)$ for subject $j$ of family $i$ that is linearly dependent on the frailty, i.e.
\[
\lambda_{x_{ij}}(t\mid r_i) = r_i\lambda_0(t; x_{ij}),
\] \cite{duchateau2008frailty, ripatti2000estimation}. 
Notice that this readily coincides with the model specification \[ S_{x_{ij}}(t\mid r_i) =  [S_0(t;x_{ij})]^{r_i}, \] where $S$ indicates the survival function. Notice that for identifiability, $\mathbb{E}(R) = 1$ is usually assumed, such that this is allowed by the  frailty distribution.

As already mentioned above, the joint survival and hazard function within a family can be defined through the conditional independence assumption \cite{rodriguez2010multivariate}. Consider two women who belong to the same family. Let $T_1 = t_1$ and $T_2 = t_2$ be the time-to-event of the two women, and again let $R = r$ be their  family-specific frailty. Then by the assumption of conditional independence one has that the joint survival function factorizes conditional to the frailty risk \[
S_{12}(t_1,t_2\mid r) \overset{T_1\perp T_2\mid R}{=} S_1(t_1\mid r)S_2(t_2\mid r).
\] 
The random effect $r$ has the following multiplicative effect on the two cumulative hazard functions $\Lambda_1(t)$ and $\Lambda_2(t)$ of the two women:
\begin{align*}
    \lambda_i(t\mid r) = r\lambda_{0i}(t), \quad \Lambda_i(t\mid r) = r\Lambda_{0i}(t), \quad S_i(t\mid r) = S_{0}(t_i)^{r}
\end{align*} for $i = 1,2$ and where $\Lambda_{0i}$, as we have seen, can depend on subject-specific covariates. 

Thus, the unconditional joint survival function is equal to \begin{align*}
...
\end{align*} where $\mathcal{L}_g(r)$ is the Laplace transform of the density function $g(r)$ of $R$ evaluated at $\Lambda_{01}(t_1)\Lambda_{02}(t_2)$. To obtain the explicit form of $S_{12}(t_1,t_2)$ one should specify $g(r)$, that is typically a Gamma or Lognormal distribution. This framework can be easily extended to more than two subjects \cite{hougaard2012analysis}, and indeed the term ``multivariate'' is utilized to refer to many time-to-event outcomes within a single family. 

In contrast, in the semiparametric setting, the Cox models enable us to utilize a partial likelihood maximization technique \cite{lawless2011statistical}, which allows us not to specify the distribution of the baseline hazard function and thus is more adaptable but has the drawback of not computing closed-form solutions for relevant quantities, and of a poorer interpretation of the phenomenon of interest.
...
Let $i=1,\dots,n$ identify the family and the main subject of a family, and $\{m,s_1\}$ to identify the two family members ``mother'' and ``first sister'', respectively. The generalization of this to families with more than three members is straightforward, as we will see also in the application to the available data. The observed survival data is $\underline X=\left(\underline X_1,\dots,\underline X_n\right)^T$, where $\underline X_i=(\underline x_i, \underline{xs}_{1i}, \underline xm_i)^T$. 
For the generic subject $i$, $\underline x_i=(x_{i}=\text{min}(t_{i}, c_{i}),\delta_{i})^T$, $\delta_{i}=\mathbb{I}(t_{i} \le c_{i})$ following the usual notation that has $t_i$ indicate the survival time, and $c_i$ indicate the administrative (independent) censoring time, both measured from the same origin, that in our case is birth, indexed by $b_i$. The notation for the other family members is obtained by having $x, \ t, \ c, \ b$, and $\delta$ be followed by $m$, and  $s_1$ as shown above. If the $i$th subject does not have a sister, then the sister birthday is set as $bs_{1i} = +\infty$ and consequently the time-to-event is $ts_{1i} = +\infty$. The distinction between mother and sister is not strictly needed here as we assume that their age-at-onset distributions are equal but it will make the extension to more complex models easier. Additionally, it would be interesting to generate a targeted survival function based on the generation of each member.

Recall the proportional hazard (PH) of Lehmann structure, which is defined by the fact that the hazard function in a risk group can be written as a function of the baseline hazard function $\lambda_0(t)$ and the risk, as $\lambda_r(t) = r \lambda_0$(t), with the baseline hazard function $\lambda_0(t)$ be distributed according to a distribution that may be identified by a parameter, say $\theta$. We consider the Lehmann structure in terms of the survival functions $S_r(t)=[S_0(t)]^r$, and we extend it to the model defined as the more general Lehmann Cure Rate (LCR) model obtained by applying the Lehmann power transformation to a Cure Rate (CR) survival function \[CR: \ S_0(t) = p + (1-p) \widetilde{S}(t)\] 
\[LCR: \ S_r(t) = \left[ p + (1-p) \widetilde{S}(t) \right]^r, \ \ r >0,  \] with $\widetilde{S}(t)$ a proper survival function which describes the time-to-event distribution of subjects who will eventually experience the event (we call these the ``cases''). Thus, the time-to-event can not in this case admit the value $+\infty$, and the survival function $\lim_{t\to+\infty}\widetilde S(t) = 0$. Note that for a fixed value $r$, the model is such that the survival function $S_r(t)$ also defines a cure rate model. Indeed, $\lim_{t \rightarrow +\infty} S_r(t) = p^r$, and $S_r(t)$ can thus be written as
\begin{eqnarray}
...
\label{condsurv}
\end{eqnarray}
and proper conditional density function for the cases 
\begin{eqnarray}
...
% Moreover, we make some additional assumption about the survival data, that we will use also later in the Section \ref{sec5}. If the $i$th subject does not have a sister, then the sister birthday is set as $bs_{1i} = \infty$ and consequently the time-to-event is $ts_{1i} = \infty$. The extension to multiple sisters easily follows the same structure. 

% We want to prove that a Multivariate Survival model, with the information on the family risk $R$ not available, has better performance than the model where we replace the unknown frailty with a known parameter. 
% Additionally, we will prove the crucial function that families have in assessing the role of the frailty, so we also show the Univariate version of the former model said.

% The vanilla version of a multivariate shared frailty survival model, since it only admits the existence of two risk groups, with conditionally independent survival times within each family. The extension to k-groups of infinite groups is straightforward and shown afterward. Recall the aforementioned notation.

We now construct the observed data likelihood function $L(\underline{\pi};\underline X)=\prod_{i=1}^n f_X(\underline X_i;\underline{\pi})$, with $\underline{\pi}=\{p, \underline\gamma,\theta\}^T$, where $p$ is the fraction of ``non-cases'' into the population, $\underline{\gamma}$ is the parameter of the baseline survival distribution of cases when one makes a parametric assumption about its distribution, and $\theta$ is the frailty parameter.

\subsection{Parametric and Semiparametric Gamma Frailty models}\label{sec:1.3.1}
Incorporating a continuous frailty term can increase the variability among the possible values that the latent quantity may take. It is important for this quantity to be positive, as it represents an individuals' risk of developing breast cancer during their lifetime. We assume the frailty to follow a Gamma($shape = \theta$, $rate = \theta$) whose density function is given by \begin{align*}
    g_R(r) = \dfrac{\theta^\theta}{\Gamma(\theta)}r^{\theta-1}\text{e}^{-r\theta}.
\end{align*}

% As a reminder, the survival functions that we utilize are: \begin{align*}
%     &S_r(z_i) = S_0(z_i)^r = [p+(1-p)\widetilde S(z_i)]^r \\
%     &f_r(z_i) = \dfrac{\partial \left[S_0(z_i)^r\right]}{\partial z_i}= (1-p)\widetilde f(z_i) r[p+(1-p)\widetilde S(z_i)]^{r-1}
% \end{align*} where we call $S_r(t)$ and $f_r(t)$ the cure rate survival and density function conditional to the frailty, i.e. $S(z_i\mid r)$ and $f(z_i\mid r)$. These functions are involved in the likelihood, such that $\underline{\pi}$: 
% \begin{align}
%     \label{}
%     &L^*(\underline{\pi};\underline{\mathbf{z}}) =\prod_{i=1}^n f_Z(\underline{\textbf{z}}_i;\underline{\pi}) = \prod_{i=1}^n\int_{\mathbb{R}^+}f_Z(\underline{\textbf{z}}_i\mid R;\underline{\pi})g_R(r)\partial r \\
%     & \qquad\qquad=\prod_{i=1}^n\int_{\mathbb{R}^+}f_r(z_i)^{\delta_i}S_r(z_i)^{1-\delta_i}g_R(r)\partial r. \nonumber 
% \end{align} for the parameter collection $\underline{\pi}$.

First note that one can rely on the Cox proportional hazard model if one is willing to ignore the cure rate structure. Although the true underlying structure of this model is based on the CR Lehmann structure, one may think of a model that has a proper survival distribution, both conditionally on $R$ and marginally. In other words, we may assume that there exists some finite time $T_\text{max}$ (larger than all observed survival and censoring times) such that all conditional survival functions $S(t\mid r) = \left[S_0(t) \right]^r$ are proper, i.e. tend to zero as $t\to+\infty$. Then, semiparametric estimation of the model is possible, for example using the \texttt{emfrail} function available in the software \texttt{R} \cite{balan2019frailtyem, balan2020tutorial}, while maintaining the multiplicative frailty structure. One may therefore rely on the maximization of the partial likelihood for the estimation procedure, instead of using the complete likelihood, with no need to specify a parametric distribution of the baseline survival function. 

Therefore, a multivariate semiparametric Cox shared frailty clustered model, with the families our clusters, is formulated in its conventional form based on the existing literature. 

We would like to prove the superiority of the multivariate model to the univariate models. One of these last would be the univariate Cox frailty model. By definition the word ``univariate'' refers to the families that have only one subject in this setting. The problem of this model is its non-identifiability due to the combination of cluster absence - so that we can not have a ``shared frailty'' effect - and the unspecified hazard. We prove the non-identifiability of the univariate Cox frailty model in the following lines.
...
\end{align} where $S_r(t) = S(t\mid r)$ and $f_r(t) = f(t\mid r)$ are the (proper) conditional survival function and density function conditional, respectively.

We note that, however, the univariate multiplicative Gamma frailty model is not identifiable without some additional structure. We go deeper into the issue with the following lemma.
\begin{lem} The Univariate multiplicative proportional hazards frailty model is not identifiable if one does not specify the form of the baseline survival function $S_0(t)$. 
    \begin{proof}
We discuss the case of multiplicative Gamma frailty, but the proof is general. 
...
\end{align*} 
The frailty model has marginal survival function \begin{align*}
...
\end{align*} 
for any observed $S(t)$, to each choice of $\theta$ there is a corresponding $S_0(t)$. Thus, it exists an infinite set of couple $(\theta, S_0(t))$ with $S_0(t) = \text{exp}\left(\theta\left(1 - S(t)^{-1/\theta} \right)\right)$ that yields to the same $S(t)$. This proves that the model is not identifiable from the marginal survival function $S(t)$, unless additional constraints are introduced, such as for example on the distribution of $S_0(t)$.
    \end{proof}
\end{lem}
Thus, the semiparametric Cox univariate frailty model is not developed and no results are reported from this model. However, when we specify a parametric distribution for the univariate survival function, we can develop the model and present the results obtained from it, as we see below.

However in contrast, we develop two non-frailty univariate Cox model, where we replace the true frailty risk with the family history indicator $FH$. To recap, we develop the multivariate Cox model as the first model, as second model a Cox model with a covariate that coincide to the constant $FH$ indicator. The term ``constant'' refers to measuring the indicator $FH$ at the end of the follow-up, thus we do not take into account the time of the change-point of the indicator from zero to one if it happens. The third model is a Cox model with a time-varying covariate $FH(t)$. Thus, in this model it is also recorded the time of the change-point.

Our main contribution is to compare these three semiparametric models namely in short: the multivariate frailty Cox model, the constant $FH$ Cox model, and the time-varying $FH(t)$ Cox model to our proposals that consists in three parametric models. 

The model we expect to be the best performing the precision in inference and prediction accuracy is the multivariate parametric Lehmann cure rate model with shared frailty. Recall the main model components in \ref{condsurv} and \ref{formula:1.conddens}, such that
\begin{align*}
    &S_r(t) = [S_0(t)]^r = [p+(1-p)\widetilde S(t)]^r \\
    &f_r(t) = (1-p^r)\widetilde f_r(t) = (1-p)\widetilde f(t) r[p+(1-p)\widetilde S(t)]^{r-1}
\end{align*} we can find the closed form of the multivariate likelihood $L(\underline\gamma, p, \theta)$ for $i = 1,\dots, n$ families of varying size $n_i$, thus generalizing the case of three-member families seer earlier 
\begin{align*}
...
\end{align*}
Notice that parameter estimation is achieved through the maximization of the likelihood from the observed independently right-censored data. Thus, we obtain the estimated collection of parameters $\underline{\widehat\pi} = (\widehat p, \underline{\widehat\gamma}, \widehat\theta)^T$ that are consequently involved in the computation of quantities of interest.

A non-frailty cure rate Lehmann model can be compared to the parametric frailty models, recalling that the family history is used as a replacement for the true frailty risk. The survival functions in this model depend on the observed family history indicator $FH(t)$ that assumes the value zero until the first case of breast cancer is observed in the family, after which it takes the value one. Notice that the $FH$ enters in the model as a baseline covariate, and thus the survival function depends on this covariate. The $FH$ survival function is given by \begin{align*}
    ...
    &\text{Then, } \widetilde f_{FH}(t) = -\dfrac{\partial \widetilde S_{FH}(t)}{\partial t} = \dfrac{(fh(\beta-1)+1)[p+(1-p)\widetilde S(t)]^{fh(\beta-1)} (1-p)\widetilde f(t)}{(1-p^{fh(\beta-1)+1})}, \\
    &\text{and, } f_{FH}(t) = (1-p^{fh(\beta-1)+1})\widetilde f_{FH}(t)= (fh(\beta-1)+1)(1-p)\widetilde f(t) [p+(1-p)\widetilde S(t)]^{fh(\beta-1)},
\end{align*} for $FH = fh$. Notice that the frailty parameter $\theta$ is not involved anymore in this model, due to the fact that it is ``replaced'' by the parameter $\beta$, which identifies the average risk difference between the group of families with a negative family history ($FH = 0$), and those families with a positive family history ($FH = 1$). The family history conditional density function of the cases is $f_{FH}(t)$, because recall that also here the cure rate structure is involved, with cases and non-cases. Notice that when $FH = 0$ the survival function reduces to the baseline survival function $S_{FH}(t;FH=0) = [p+(1-p)\widetilde S(t)]$, otherwise $S_{FH}(t;FH=1) = [p+(1-p)\widetilde S(t)]^\beta$, which allow as well for a cure rate model as proved above. Thus, the univariate likelihood with the covariate family history indicator, on the parameter collection $\pi_{FH}=(\underline\gamma, p, \beta)^T$, is given by \begin{align*}
...
\end{align*}
Wrapping up, we explore six different models, namely in short: the multivariate semiparametric Cox frailty model, the constant $FH$ Cox model, the time-varying $FH(t)$ Cox model, the multivariate parametric frailty model, the univariate parametric frailty model, the parametric univariate $FH$ model. Our purpose is to observe their difference in inference precision and risk prediction accuracy. We expect the family history models to be weaker in comparison to all the others. Also, we expect the semiparametric models to be poorer in the explanation of the phenomenon ``breast cancer development'' in comparison to the parametric models. We would like to assess that our parametric multivariate model is the best one performing parameters recovery and accuracy in risk prediction in contract to what has been already used in the literature so far. 

\section{Posterior risk prediction}\label{sec4}
The primary focus of this project is to develop a risk prediction model that can be used for women who are not included in the training dataset. This model should also be applicable to new family members who are admitted to a hospital at a later time. The risk prediction model is evaluated based on various measures of risk prediction accuracy and validation techniques to assess which model is the most informative and effective.

% First of all we compute the posterior quantities in the parametric case of two-latent risk groups. We first explore this case, following the functions defined in Section \ref{sec2}, and later we will extend to the more difficult case of continuous frailty relying on the notation defined in Section \ref{sec3}.

% We can obtain the complete form of the survival functions, which are: \begin{align*}
...
%     &\widetilde{S}_1(t) =\dfrac{(\widehat{p}_0+(1-\widehat{p}_0)\widetilde{S}(t))^{{\widehat{\alpha}}}-\widehat{p}_0^{{\widehat{\alpha}}}}{1-\widehat{p}_0^{{\widehat{\alpha}}}}
% \end{align*} where $\widetilde{S}(t)$ has a defined distribution in function of $\widetilde{\lambda}(t)$, according to the baseline prior distribution, previously fixed. 

% Perhaps most importantly, one should not forget the final goal of our work: provide an individual-based prediction of their family's risk group. This is achieved for the univariate estimators through calculation, for each family, of the conditional probability:
% \begin{align*}
%         P(R=1\mid \underline{z};\underline{\widehat{\pi}}) &= \dfrac{f(\underline{z}\mid R=1;\underline{\widehat{\pi}})P(R=1)}{f(\underline{z})} \\
%         &= \dfrac{f(\underline{z}\mid R=1;\underline{\widehat{\pi}})P(R=1)}{f(\underline{z}\mid R=1;\underline{\widehat{\pi}})P(R=1)+f(\underline{z}\mid R=0;\underline{\widehat{\pi}})P(R=0)}.
%     \end{align*} 
% To simplify the expressions we use $\underline{z} = (z, \delta)$ to indicate the univariate survival couple data. The vector collection $\underline{\textbf{z}}=((z,\delta)^T,(zs_1,\delta s_1)^T,(zm, \delta m)^T)^T$ represents the whole family data. The purpose to define the family with at least two members, i.e. the mother and the first sister ($m$ and $s_1$ respectively) is to cover at least the first-degree-generational heritability of breast cancer. Notice again that this is easily extendable to a higher number of sisters, or other degree members of the family (see e.g. grandmothers, aunts, cousins). 

% The formula for the full multivariate model is similar, but it involves the survival information from all family members, taking into account the conditional independence assumption within each family: \begin{align}
%     \label{formula:4_1}
%         P(R=1\mid \text{family};\underline{\widehat{\pi}}) &= \dfrac{f(\text{family}\mid R=1;\underline{\widehat{\pi}})P(R=1)}{f(\text{family};\underline{\widehat{\pi}})} \nonumber \\
%         P(R=1\mid \underline{\textbf{z}};\underline{\widehat{\pi}}) &= \dfrac{f(\underline{\textbf{z}}\mid R=1;\underline{\widehat{\pi}})P(R=1)}{f(\underline{\textbf{z}})} \\
%         &= \dfrac{f(\underline{\textbf{z}}\mid R=1;\underline{\widehat{\pi}})P(R=1)}{f(\underline{\textbf{z}}\mid R=1;\underline{\widehat{\pi}})P(R=1)+f(\underline{\textbf{z}}\mid R=0;\underline{\widehat{\pi}})P(R=0)}. \nonumber
...
%     \end{align*} and:
%     \begin{align*}
%         &S_0(z) = \widehat{p}_0+(1-\widehat{p}_0)\widetilde{S}(z)  \\
%         &f_0(z) = (1-\widehat{p}_0)\widetilde{f}(z)\nonumber \\
%         &S_1(z) = [S_0(z)]^{\widehat{\alpha}} = [\widehat{p}_0+(1-\widehat{p}_0)\widetilde{S}(z)]^{\widehat{\alpha}} = \widehat{p}_0^{\widehat{\alpha}} + (1-\widehat{p}_0^{\widehat{\alpha}})\widetilde{S}_1(z)\nonumber \\ 
%         &f_1(z) = (1-\widehat{p}_0^{\widehat{\alpha}})\left(\dfrac{1-\widehat{p}_0}{1-\widehat{p}_0^{\widehat{\alpha}}}\right)\widetilde{f}(z){\widehat{\alpha}}\left(\widehat{p}_0+(1-\widehat{p}_0)\widetilde{S}(z)\right)^{\widehat{\alpha}-1} \\
%         &\text{with } \widetilde{S}_1(z)=\dfrac{(\widehat{p}_0+(1-\widehat{p}_0)\widetilde{S}(z))^{\widehat{\alpha}}-\widehat{p}_0^{\widehat{\alpha}}}{1-\widehat{p}_0^{\widehat{\alpha}}}
%     \end{align*}
...
% \begin{align}
% \label{formula:4_2}
%         &S(z+k\mid \text{family};\underline{\widehat{\pi}}) = S(z+k\mid R=1,\text{family};\underline{\widehat{\pi}})P(R=1\mid \text{family};\underline{\widehat{\pi}}) \\ 
%         &\qquad\qquad\qquad+S(z+k\mid R=0,\text{family};\underline{\widehat{\pi}})P(R=0\mid \text{family};\underline{\widehat{\pi}}). \nonumber \\ 
%         &S(z+k\mid \underline{\textbf{z}};\underline{\widehat{\pi}}) = S(z+k\mid R=1;\underline{\widehat{\pi}})P(R=1\mid \underline{\textbf{z}};\underline{\widehat{\pi}})  \nonumber \\
%         &\qquad\qquad\qquad+S(z+k\mid R=0;\underline{\widehat{\pi}})P(R=0\mid \underline{\textbf{z}};\underline{\widehat{\pi}}) \nonumber \\
%         &S(z+k\mid R=1;\underline{\widehat{\pi}}) = S_1(z+k;\underline{\widehat{\pi}})\nonumber \\
%         &S(z+k\mid R=0;\underline{\widehat{\pi}}) = S_0(z+k;\underline{\widehat{\pi}})\nonumber
% \end{align} with ``family'' referring to complete observed data of breast cancer experience of all family members, and $P(R=1\mid \underline{\textbf{z}};\underline{\widehat{\pi}})$ obtained in the previous step in \ref{formula:4_1}. Due to the target sample of this probability, i.e. non-cases women, the observed time $z$ always corresponds to the censoring time. 
\subsection{Semiparametric model}
In the semiparametric scenario the estimation procedure does not need to specify the baseline survival function. By maximizing the partial likelihood, we obtain the estimated value of the frailty parameter $\widehat{\theta}$. Note that we set $R\sim$ Gamma$(shape = \theta, rate = \theta)$, so that the expected value of the frailty is one. Furthermore, the frailty value for each family is obtained through the available \texttt{coxph} function in the \texttt{R} software, using an Empirical Bayes method implemented in the package to compute the posterior mean \cite{balan2020tutorial}.

With the value of the frailty parameter we can obtain the estimated family-specific posterior frailty density. This step is crucial for doing fast risk prediction. We aim to infer the posterior density of the frailty based on the information available from the ``new'' family, using the estimated $\widehat\theta$. The posterior distribution is given by \[
...
\] A similar procedure is applied for the other family members, and also again, this is easily extendable to a higher number of female family members.
We need an estimator $\widehat S_0(t)$ of the baseline survival function, and an estimator $\widehat\lambda_0(t)$ of the baseline hazard function such that one can estimate the density function
\[
\widehat{f}(\underline{x}\mid R = r) = (r\widehat{\lambda}_0(x))^\delta\widehat{S}_0(x)^r.
\]
The baseline cumulative hazard function, and consequently the survival function $\widehat S_0(t) = \text{e}^{-\Lambda_0(t)}$, can be estimated by the Breslow estimator $\widehat\Lambda_0(t)$. 

In the following lines, we present the classical estimation of the Breslow estimator with included covariates. It is important to note that the form we report includes the covariate, as in the FH model, the FH covariate needs to be incorporated into the model. We consider the expected number of events occurring at time $[\tau_j,\tau_{j+1}]$, given the $l$th subject ``at risk'' at time $\tau^-_j$, who belongs the risk group defined as $R(\tau_j)$. Thus, we have \begin{align*}
...
\end{align*} When setting the aforementioned quantity equal to the observed number of events $d_j$ until time $\tau_{j+1}$, it yields \begin{align*}
...
\end{align*} where $\underline{x}_l$ is the covariate vector for $l$th subject, and $\widehat{\Lambda}_0(t)$ is the Breslow estimator. Notice that here the frailty quantity is not involved. The estimated frailty parameter and the Breslow estimator are combined together afterward. 

The Breslow estimated value of the survival function is obtained through the available function \texttt{predictCox}, which takes as input a Cox model, in the package \texttt{riskRegression} \cite{ozenne2017riskregression} of the software \texttt{R}, while the estimated hazard function can be obtained in the function \texttt{coxphHaz} of package \texttt{biostat3} \cite{biostat3}.

In order to compute the frailty posterior distribution, the estimated hazard is needed. However, the problem can be simplified by relying on the conjugacy property of the Gamma prior for this model. Specifically, the posterior distribution of the frailty for a particular family is easily shown to be distributed as a Gamma distribution with shape parameter equal to the sum of the indicator of observed events in the family $d_i$, in addition to the frailty parameter $\theta$, and rate parameter equal to the sum of the cumulative hazard function evaluated at the event times of the family members and the frailty-specific hazard function evaluated at time zero, i.e. $\Lambda_0(\underline x_i) = \sum_{j=1}^{n_i}\Lambda_0(x_j)$, where $\underline x_i = (x_1, \dots, x_{n_i})^T$ for the $n_i$ subjects in a family. If the covariate information is included in the model, then $\Lambda_0(x_i)$ translates to the sum of the cumulative hazard function evaluated at the event times of the family members multiplied by the exponential function of the covariate $\underline{x}_{ij}'\underline{\beta}$, such that $\Lambda_0(x_i) = \sum_{j=1}^{n_i}\Lambda_0(x_j)\text{exp}(\underline{x}_{ij}'\underline{\beta})$. Indeed, for three-member families we have \begin{align*}
...
\end{align*}
where $h_i(\theta) = d_i+\theta, \ l_i(\theta) = \theta + \sum_{j=1}^{n_i}\widehat{\Lambda}_0(x_j)$. At the end of the day we will use only the estimated cumulative hazard. 
This formula is also computed with a different parametrization of the Gamma distribution in the literature \cite{duchateau2008frailty}, where it is used in the maximization step of the EM algorithm involved in the maximization process for parameter estimation. Recall that the EM algorithm can be applied to the partial Cox likelihood to obtain the parameter estimates, due to the presence of the frailty quantity.
% spiega meglio cos'e' la partial likelihood con frailty quantity 
The final formula is given by \begin{align*}
...
\end{align*} 
Notice that to obtain the posterior frailty distribution of each family we need to compute the cumulative hazard function at the observed times for that family. If the cumulative hazard (Breslow) value is not available for that time point, we use linear interpolation on the available data points \cite{meijering2002chronology}.

Typically, the posterior frailty distribution has a poor meaning to the end-user. We need to communicate to each family a meaningful summary of it. We can rely on the usual summaries such as the mode, median or mean of the posterior frailty density, whose form are give by \begin{align*}
...
\end{align*}
The mode of a Gamma distribution is null when $\widehat{\theta}$ has a value lower than one. The mode will therefore not be a good indicator of the posterior frailty density because one may observe several zero values across the families. We therefore exclude it as a valid indicator. 

From above, the family-specific estimated posterior mean is immediately obtained withe the form  \begin{align*}
...
\end{align*} where $w_i = \dfrac{\widehat{\Lambda}_0(\underline x_i)}{\widehat{\theta} + \widehat{\Lambda}_0(\underline x_i)}$ is the weight of the weighted average of the ratio between the observed events $d_i$ and the sum of the cumulative hazard by families $\widehat{\Lambda}_0(\underline x_i)$ 
% that can be interpreted as the expected number of events 
and one, the prior frailty mean. 

The median is computed numerically quite easily from the estimated family-specific posterior distribution.
...
We therefore introduce another proposal. Given that the posterior mean does not really have a scale of reference, we compute the posterior probability of belonging to the high-risk group, i.e. the probability that the family-specific frailty value is greater than a quantile of the prior distribution of $R$ over the population, evaluated using $\widehat\theta$. Setting a percentile value for assessing the highest-risk family to $\alpha$, this is \begin{align*}
    P(R \ge F^{-1}_R(\alpha; \widehat{\theta})\mid \text{family data}; \widehat{\theta}) 
    % = 1 - F_{R_\text{post}}(\widehat{r}\mid \text{family}; \widehat{\theta})
 \end{align*}
From this probability, one may proceed to a binary classification of families into two risk group (low and highest-risk group), fixing a threshold of choice, and make a comparison with the true belonging to the high-risk group. The receiver operating characteristic (ROC) curve with the area under the curve (AUC), that  the positive predictive value (PPV) and the negative predictive value (NPV) give us a validation measure of the binary splitting made by the models. Clearly, this validation procedure can be applied only in the case of simulation studies to validate the prediction model, since the true family-specific frailty needs to be known. 
From this probability, one may proceed to a binary classification of families into two risk group (low and highest-risk group), fixing a threshold of choice, and make a comparison with the true belonging to the high-risk group. The ROC curve with the area under the curve (AUC), that illustrates the trade-off between the true positive rate (sensitivity) and the true negative rate (specificity) at various classification thresholds is computed. Also, the positive predictive value (PPV) and the negative predictive value (NPV) give us a validation measure of the binary splitting made by the models. Clearly, this validation procedure can be applied only in the case of simulation studies to validate the prediction model, since the true family-specific frailty needs to be known. 

In the application presented in Section \ref{sec:1.6}, we focus on utilizing the posterior distribution of the frailty mean. Specifically, we calculate the chosen threshold at the conventional significance level of $\alpha = 0.05$, which corresponds to the prior percentile of belonging to the highest-risk families. Using this threshold, we can determine the probability of a new family value exceeding the threshold prior percentile. 
%Later, we compare this last value to the threshold posterior percentile of the population distribution previously estimated on the data and later saved. 

\subsection{Parametric model}\label{sec:1.4.2}
In contrast, in the parametric scenario, the baseline survival function is available directly through its estimated parameters, and that enables us to compute the posterior frailty distribution conditional on the whole family data in a parametric manner, also taking into account the explicit cure rate structure of the model. This allows one to obtain the posterior mean, mode, and median as useful summaries of the posterior frailty distribution. By estimating a threshold quantile from the posterior population frailty distribution, we can calculate a new family's posterior probability distribution of the frailty and the indicator of belonging to the highest-risk families. The family-specific posterior frailty distribution conditional on the whole family data is given by \begin{align*}
    ...
    &\text{which is Gamma}\left(shape = \alpha + \sum_{j=1}^{n_i}\delta_j, rate = \beta -\sum_{j=1}^{n_i}\log\left(p + (1-p)\widetilde S(x_j)\right)\right)
\end{align*} The density function, in the univariate case, reduces immediately to \begin{align*}
...
\end{align*} 
which is a Gamma$\left(shape = \alpha + \delta, rate =\beta - \log(p + (1-p)\widetilde S(x))\right)$.

The posterior mean and mode frailty are extracted by the formula on the Gamma distribution. They are given by \begin{align*}
    \text{Mean} = \dfrac{\alpha + \sum_{j=1}^{n_i}\delta_j}{\beta - \sum_{j=1}^{n_i}\log\left(p + (1-p)\widetilde S(x_j)\right)}, \\
    \text{Mode} = \dfrac{\alpha + \sum_{j=1}^{n_i}\delta_j - 1}{\beta - \sum_{j=1}^{n_i}\log\left(p + (1-p)\widetilde S(x_j)\right)},
\end{align*} where the univariate case is obtained setting $n_i=1$. Note that the posterior median of the frailty distribution requires numerical methods to be computed as there is not a closed form formula for the Gamma distribution. 
Instead, about the mode, it is important to notice that when the shape parameter of the distribution is less than one, the mode is not a reliable summary of the posterior frailty and is in fact the weakest indicator among all.

% the previously used method of the moment generating function. Thus we finally obtain \begin{align*}
%     &\mathbb{E}(r\mid \text{family}) = \dfrac{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j+1)}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}\beta}\left(1-\dfrac{\text{log}\prod_{j=1}^{n_i}\left(p + (1-p)\widetilde S(z_j)\right)}{\beta}\right), \\ 
%     &\mathbb{E}(r\mid \text{subject}) = \dfrac{\Gamma(\alpha + \delta+1)}{{\Gamma(\alpha + \delta)}\beta}\left(1-\dfrac{\text{log}\left(p + (1-p)\widetilde S(z)\right)}{\beta}\right),
% \end{align*} respectively for the multivariate and the univariate case. The computation in the multivariate case is given by \begin{align*}
%     &\mathbb{E}(r\mid \text{family}) = \int_{\mathbb{R}^+} r g_R(r\mid \textbf{z})\text{d}r = \\
%     &\int_{\mathbb{R}^+} r \left(1 - \dfrac{\text{log}\prod_{j=1}^{n_i}(p + (1-p)\widetilde S(z_j))}{\beta}\right)^{-(\alpha + \sum_{j=1}^{n_i}\delta_j)}\cdot \\
%     &\cdot\prod_{j=1}^{n_i}\left(p + (1-p)\widetilde S(z_j)\right)^r\dfrac{\beta^{\alpha + \sum_{j=1}^{n_i}\delta_j}}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}}r^{\alpha + \sum_{j=1}^{n_i}\delta_j-1}\text{e}^{-\beta r} = \\
%     &= \left(1 - \dfrac{\text{log}\prod_{j=1}^{n_i}(p + (1-p)\widetilde S(z_j))}{\beta}\right)^{-(\alpha + \sum_{j=1}^{n_i}\delta_j)}\cdot\\ 
%     &\cdot\int_{\mathbb{R}^+}\prod_{j=1}^{n_i}\left(p + (1-p)\widetilde S(z_j)\right)^r \dfrac{\beta^{\alpha + \sum_{j=1}^{n_i}\delta_j}}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}}r^{\alpha + \sum_{j=1}^{n_i}\delta_j+1-1}\text{e}^{-\beta r}\text{d}r
% \end{align*} where only the integral part can be solved by \begin{align*}
%     &\int_{\mathbb{R}^+}\prod_{j=1}^{n_i}\left(p + (1-p)\widetilde S(z_j)\right)^r \dfrac{\beta^{\alpha + \sum_{j=1}^{n_i}\delta_j}}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}}r^{\alpha + \sum_{j=1}^{n_i}\delta_j+1-1}\text{e}^{-\beta r}\text{d}r = \\
%     &=\int_{\mathbb{R}^+}\text{e}^{r\text{log}\prod_{j=1}^{n_i}(\left(p + (1-p)\widetilde S(z_j)\right))}\dfrac{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j+1)}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}\beta}\cdot \\
%     &\cdot\dfrac{\beta^{\alpha + \sum_{j=1}^{n_i}\delta_j+1}}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j+1)}}r^{\alpha + \sum_{j=1}^{n_i}\delta_j+1-1}\text{e}^{-\beta r}\text{d}r \\
%     &=\dfrac{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j+1)}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}\beta}\text{MGF}\left(\text{log}\prod_{j=1}^{n_i}\left(p + (1-p)\widetilde S(z_j)\right);R^*\right) \\
%     &= \dfrac{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j+1)}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}\beta}\left(1 - \dfrac{\text{log}\prod_{j=1}^{n_i}(p + (1-p)\widetilde S(z_j))}{\beta}\right)^{\alpha + \sum_{j=1}^{n_i}\delta_j + 1}, 
% \end{align*} with $R^*\sim\text{Ga}(shape=\alpha+\sum_{j=1}^{n_i}\delta_j+1,rate=\beta)$. Thus, \begin{align*}
%     \mathbb{E}(r\mid \text{family}) = \dfrac{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j+1)}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}\beta}\left(1 - \dfrac{\text{log}\prod_{j=1}^{n_i}(p + (1-p)\widetilde S(z_j))}{\beta}\right).
% \end{align*} 

% The median instead might be computed empirically. The form involving the entire family would it be given by solving \begin{align*}
%     &\int_0^r g_R(r\mid \textbf{z}) = 0.5 \\
%     &\int_0^r \dfrac{\prod_{j=1}^{n_i}\left(p + (1-p)\widetilde S(z_j)\right)^r\dfrac{\beta^{\alpha + \sum_{j=1}^{n_i}\delta_j}}{{\Gamma(\alpha + \sum_{j=1}^{n_i}\delta_j)}}r^{\alpha + \sum_{j=1}^{n_i}\delta_j-1}\text{e}^{-\beta r}}{\left(1 - \dfrac{\text{log}\prod_{j=1}^{n_i}(p + (1-p)\widetilde S(z_j))}{\beta}\right)^{\alpha + \sum_{j=1}^{n_i}\delta_j}} = 0.5. \\
%     &\int_0^r \dfrac{\prod_{j=1}^{n_i}\left(p + (1-p)\widetilde S(z_j)\right)^r
%     g_R(r\mid\sum_{j = 1}^{n_i}\delta_j;\alpha,\beta)}{\left(1 - \dfrac{\text{log}\prod_{j=1}^{n_i}(p + (1-p)\widetilde S(z_j))}{\beta}\right)^{\alpha + \sum_{j=1}^{n_i}\delta_j}} = 0.5. 
% \end{align*} with $R\sim$Gamma$(shape = \alpha + \sum_{j = 1}^{n_i}\delta_j, rate = \beta)$. We build the following algorithm for one family: \begin{itemize}
%     \item[1)] sample $r_k$; 
%     \item[2)] compute and store $g_R(r_k \mid\textbf{z};\text{estimated parameters})$ for each $r_k$ at step 1); 
%     \item[3)] the median is estimated by 
%     \[
%     r_k:\arg\min_{\mathbb{R}^+}\left(\sum_{k}g_R(r_k \mid\textbf{z};\text{estimated parameters}) - 0.5\right)^2.
%     \]
% \end{itemize} More easily, the posterior frailty mode is computed through the available software by solving \begin{align*}
%     \text{Mode} = r:\arg\max_{\mathbb{R}^+}g_R(r \mid\textbf{z};\text{estimated parameters}).
% \end{align*} 

The accuracy of the risk prediction is assessed by different indices according through simulation studies or available data. In the case of simulation studies we can compare the true generated frailty values $\widehat r_i$ to the family-specific predictions, for $n$ families, through the estimated mean squared prediction error (MSPE): \begin{align*}
    MSPE = \dfrac{1}{n}\sum_{i=1}^n(\widehat r_i - r_i)^2.
\end{align*} 
% In alternative we can compute the adjusted quantities: \begin{align*}
%     MSPE_{\text{adj}} = \mathbb{E}\left[\dfrac{(\textbf{r} - \widehat{\mathbf{r}})^2}{\mathbf{r}^2}\right], 
% \end{align*} and compute a non-parametric regression on them versus $r$. ANCORA DA FARE  
We can also compute several indices to compare the performance of the models. Firstly, we compute the linear coefficient $\widehat{r}$ of the true frailty values $r_i$ versus the predicted frailty $\widehat r_i$ using linear regression, and report this coefficient. Additionally, we calculate the linear correlation coefficient of Pearson from the same linear regression. An alternative comparison can be performed through the rank correlation coefficient. 

For assessing prediction accuracy in simulation studies, we also report the AUC, that is the area under the receiver operating characteristic (ROC) curve. This measure assesses the predictive accuracy for a binary outcome, where the frailty is split into two groups based on a threshold.
% Specifically, the ROC curve measures the pair-wise probability that an individual who has experienced the event has a higher predicted risk than an individual who has not yet experienced the event. 

When moving to the available dataset, the aforementioned comparison with the true frailty is not feasible and indeed, we turn to the Harrell's concordance index as a measure of model performance for unknown outcomes \cite{rahman2017review, harrell1982evaluating, scheike2015estimating, tokatli2011developing}. This index estimates the probability that, among a randomly selected pair of subjects with different right-censored survival times and predicted frailty values, the one with a lower risk score (here, the predicted frailty) will outlive the other \cite{harrell1982evaluating}. Harrell's concordance index is based on all pairs of ``main subjects''that we identify one in each family for whom the ordering in the survival times can be established, taking into account right censoring. Notice that this is similar to what is done in the modified version of the Wilcoxon test on survival data with right censoring. 


The estimate is based on the proportion of concordant pairs, following the above definition, over all pairs (including discordant and tied pairs). The formula is given by \cite{schmid2016use}
\[
C = \dfrac{\sum_{i\ne j}\mathbb{I}(\widehat{r}_i < \widehat{r}_j)\mathbb{I}(x_i > x_j)\delta_j}{\sum_{i\ne j}\mathbb{I}(x_i > x_j)\delta_j},
\] where $\widehat{r}_i$ and $\widehat{r}_j$ denote the predicted frailty for two subjects $i$ and $j$, respectively, who are not members of the same family. It is of interest to notice that from theoretical result the Harrell's concordance index does not depend on the baseline survival function, but only on a random variable $Y^*\sim\text{Beta}(\theta + 1,\theta)$, such that \[
C(\theta) = S_{Y^*}\left( \frac{1}{2} \right)
\]
You can find an extended version of this result in Appendix \ref{appendix:1.c}.

\section{Simulation study}\label{sec5}'
In this section we present the results of comparing the Cox models and the parametric cure rate models with several baseline survival distributions, including Weibull, Gamma, and Lognormal distributions. Based on our experience with the Swedish Multi-generational breast cancer dataset we also added a threshold parameter to the Gamma and Lognormal distributions for improved accuracy in modeling and we include these two non-cases distributions to the analysis.

Recall that we examine the Cox model within the context of multivariate frailty or in the univariate setting, considering the family history as either a baseline ($FH$) or time-varying covariate ($FH(t)$). The family history covariate is derived from the multivariate data generation process. While, we examine the parametric models either in the multivariate frailty setting or in the univariate frailty setting, and finally with the baseline covariate $FH$. The models are a total of six.

This section is organized as follows: first, we describe the key steps of the data generation process. Next, we present the results of fitting three semiparametric models, and later three parametric models using five different time-to-event distributions, and compare their goodness of fit. Finally, we evaluate the predictive accuracy of each model through several appropriate measures.

A critical aspect to consider is our use of a cure rate survival model \cite{mota2022new}. Recall that such a model accounts for the presence of a ``cured'' population (or ``non-cases'') which comprises individuals who will never experience the event of interest, regardless of how long they live. In contrast, the remaining population, referred to as ``cases'', has a non-zero probability of eventually experiencing breast cancer onset. Given this duality in survival, distinct methods for generating survival times are required for cases and non-cases.
...
Recall, also from above, the general Lehmann family of distributions that defines a proportional hazards (PH) structure:
\begin{equation}
\left\{  S_{\alpha}(t) = \left[ \widetilde{S}_0(t) \right]^{\alpha}, \ \ \alpha >0  \right\},   
\label{formula:1.PH}
\end{equation}
where $\widetilde{S}(t)$ is the (proper) baseline survival function and the term $\alpha$ modifies it to become the (also proper) survival function $S{\alpha}(t)$. Let all random variables in the family be absolutely continuous random variables. It is then easy to check that, with obvious notation, $\lambda_{\alpha}(t) = \alpha \lambda_0(t)$ for any choice of (positive and finite) $\alpha$. When $\alpha$ is modelled through a regression structure one has the celebrated Cox proportional hazards survival model \cite{cox1972regression}.

We extend (\ref{formula:1.PH}) to the model defined by the more general Lehmann cure rate family obtained by applying the Lehmann power transformation to a cure rate survival function:
\begin{equation}
\left\{  S_{\alpha}(t) = \left[ p + (1-p) \widetilde{S}(t) \right]^{\alpha}, \ \ \alpha >0  \right\}.
\label{formula:1.LehmannCR}
\end{equation}
For a fixed value $\alpha$, the model (\ref{formula:1.LehmannCR}) is such that the survival function $S_{\alpha}(t)$ also defines a cure rate model. It is easy to check that $\lim_{t \rightarrow \infty} S_{\alpha}(t) = p^{\alpha}$, and that $S_{\alpha}(t)$ can be written as
\[
S_{\alpha}(t) = p^{\alpha} + \left( 1-p^{\alpha} \right) \widetilde{S}_{\alpha}(t)
\]
with conditional (proper) survival function for the cases equal to
\[
...
\begin{eqnarray}
\widetilde{f}_{\alpha} (t) = - \frac{d}{d t} \widetilde{S}_{\alpha}(t) = \frac{1-p}{1-p^{\alpha}} \alpha \left[ p + (1-p) \widetilde{S}(t) \right]^{\left(\alpha -1 \right)} \widetilde{f}(t).  
\label{formula:1.conddens2}
\end{eqnarray}
In this model, the term $\alpha$ can depend on a latent or observed covariate $R$, so that $\alpha = \alpha(R)$, that in our case it can be seen as the family-specific risk.

\subsection{Data generation from the Lehmann Cure Rate model with multiplicative gamma frailty}
The results from the previous subsection can be easily extended to the generation of samples from the CR model with, say, three-parameter Gamma distribution $\widetilde{S}(t)$ for the cases, when the model is enlarged to allow for a continuous multiplicative Gamma$(\theta, \theta)$ frailty $R$. Recall that the three-parameter Gamma distribution$(shape, scale, threshold)$ is also known as the Pearson type III distribution (see, e.g., \cite{bobee1991risk}). We report this example to extend the data generation process to its maximum dimension of the parameter collection. 

This model takes the Lehmann form
\[ S(t; r) = \left[ S_0(t) \right]^r {\rm,\  with\ } S_0(t)=\left[ p_0+(1-p_0) \widetilde{S}(t) \right], \ i.e.\ \alpha(r)=r \ {\rm for}\  r >0. \]

To generate survival times from this model (with set parameter values $\theta, p_0$, scale, shape, and threshold), one may proceed as follows:
\begin{enumerate}
\item generate a family-specific (or subject-specific if the model is univariate) frailty term $r$ from the Gamma$(shape = \theta, rate = \theta)$ distribution;
\item Generate a subject-specific value $u$ from the Unif$(0,1)$ distribution;
\item {\sf if} $u \leq p_0^r$
\begin{itemize}
\item[] {\sf then} set $t = + \infty$ (a non-case);
...
\[ t = \widetilde{S}^{-1} \left( \frac{u^{1/r} - p_0}{1-p_0} \right)=\widetilde{F}^{-1} \left( \frac{1-u^{1/r}}{1-p_0} \right), \]
 with $\widetilde{F}^{-1}( \cdot)$ the quantile function of the three-parameter Gamma distribution. We easily find the two-parameters distributions in the basic \texttt{R} software, while the package \texttt{FAdist} contains functions for the cumulative distribution function (CDF), density function, quantile function, and random generation from the three-parameters distribution. 
\end{itemize}
\end{enumerate}
Generation for the multivariate version of this model, i.e. from the shared multiplicative frailty cure rate model, follows easily from the conditional independence assumption within each family, using for all family members the same frailty value $r$. Lastly, independent (typically, administrative) right censoring of all observations can be applied to obtain the observed data. If one takes into account the fact that family members from different generations have different birth dates (hence in calendar time), follow-up is clearly longer for members born earlier. 
\newline \\ \newline
{\bf Note:} as we have mentioned the true, underlying structure of this model is based on the CR Lehmann structure, the fact that one does not ever observe any survival times beyond some limited time $T^*$ (when follow-up stops) is such that semiparametric estimation of the model is also possible, for example with the \texttt{R} function \texttt{emfrail}. Indeed, beyond time $T^*$ the baseline survival distribution $S_0(t)=\left[ p_0+(1-p_0) \widetilde{S}(t) \right]$ (and therefore all the frailty-specific survival distributions $S(t; r) = \left[ S_0(t) \right]^r$ might tend to zero instead to $p_0^r$ as $t \rightarrow \infty$, while still maintaining the multiplicative frailty structure. In other words, given the necessarily finite follow-up, one may assume (perhaps not realistically) that $p_0=0$ and perform semiparametric inference. This same argument allows the estimation of models within the Lehmann CR structure semiparametrically through a traditional Cox PH model for the estimation of the effect of the frailty risk and the observed covariates.

Lastly, note that for non-cure rate models the data generation would typically be simpler. For example, if $S_r(t)=[S_0(t)]^r$ with $S_0(t)$ the survival function of the Weibull($k,\lambda$) random variable, it is immediate to check that $T\mid R = r\sim$Weibull$\left( k, \lambda\sqrt[k]{r}\right)$. Usually, a reparametrization dependent on the frailty risk is sufficient to obtain the updated distribution. 
\newline \\ \newline
% Consider $S^*(t)$ the time distribution on cases. When the distribution $S^*(t)$ is not as trivial as the -- typically not useful -- exponential distribution, generation of observations require inversion of the survival function $S^*(t)$. This may not be easy to do analytically, and typically requires numerical integration from the conditional density function $f^*(t)$ of cases itself. Numerical inversion would then be needed to produce samples from the distribution.

% A faster (and still precise) algorithm can be obtained by recalling the well known distribution of the main quantities we use: recall that since any cumulative distribution function $F_T(t)$ of $T$ is distributed according to a Uniform $U=F_T(t)\sim U(0,1)$, the same result holds for any survival function $S_T(t)$ of $T$, $U=S_T(T) \sim U(0,1)$. One may indeed generate values $u$ from the $U(0,1)$ distribution and invert $S_R(t)$ directly to produce the value $t=S_R^{-1}(u)$. Given the nature of the random variable $T$, however, one should produce the value $T=\infty$, the time of the controls who will never experience the event, whenever $u< p_r=p_0^{\alpha(r)}$, with $p_0$ the probability of a non-case and $r$ the frailty value previously sampled from the population (prior) frailty distribution Gamma($shape = 1/\theta$, $\theta$), and solve $u=S_R(t)=[S_0(t) ]^{\alpha(r)}$ for $t$ in the case $u>p_r=p_0^{\alpha(r)}$. Recalling the shape of $S_0(t) = p_0 + (1-p_0)S^*(t)$, with $S^*(t)$ a proper survival function, it is easy to check that
% \[
% t= \widetilde S^{-1} \left( \dfrac{u^{1/\alpha(r)}-p_0}{1-p_0}  \right) = \widetilde F^{-1} \left( \dfrac{1-u^{1/\alpha(r)}}{1-p_0}  \right),
% \]
% which can be easily computed from the quantile function, available in most software packages for a large number of distributions (one just needs to make sure that the quantile function is never invoked for $u<p_0^{\alpha(r)}$). The proper survival function $\widetilde S(t)$ can be distributed according to any proper form and we explore the five we have already said about. We easily find the software packages for the two-parameters distributions, while the package \texttt{FAdist} contains functions for the cumulative distribution function (CDF), density function, quantile function, and random generation from the three parameter distributions. 

% In the end, we prioritize the three-parameters Gamma for its high goodness of fit and the flexibility we may gain with the additional parameter. This distribution on the cases, also called the Pearson type III distribution (see, e.g., \cite{bobee1991risk, singh2022generalized}) ADD JOHNSON AND KOTZ HERE IF YOU FIND THE DENSITY THERE!, allows for a continuous multiplicative gamma ($shape = 1/\theta$, $scale = \theta$) frailty $R$. And, in addition but not less importantly, the \texttt{R} package \texttt{FAdist} contains functions for the cdf, density function, quantile function, and random generation from the distribution. 


% The Lehmann cure rate structure allows for semiparametric estimation of models through a Cox proportional hazards model for estimating frailty and the effect of observed covariates. However, the cure rate assumption conflicts with the necessary proportional hazard assumption in the Cox model setting, as it does not allow for infinite time-to-event. To solve this issue, an upper-bound of the observed time, $T = 10000$, is set. This value is sufficiently large to enable the cure rate model to account for the usual end of life-time, while assuming that proper survival behavior is observed after that time, with all subjects eventually experiencing the event. 

Beyond the time-to-event, we need to generate birth times and censoring times. Birth times of the family members were generated as values from Uniform distributions with a lower-bound and an upperbound. First, the birth time of mothers is generated from a Uniform on years between the 1905 and the 1945. Then, the birth times of the main subject and all sisters have been generated summing up to the birth time of the mother, a value from a Uniform between 25 and 35, which coincides to the age the mother gave birth. Administrative right censoring was generated as the minimum between death time (which similarly follows the generation of birth times) and 2020, the end of the follow-up year.

For data generation, we also need to set some fixed parameter values ($p, shape$,
$scale, threshold, \theta$) = (0.85, 8, 6, 15, 0.2), where $p$ is the probability of being a ``non-case'', the $shape, scale, threshold$ are the three parameters of the survival times of the ``cases'' -- notice that when we use a two-parameter distribution we only use the first two of them: $shape, scale$ --, and $\theta$ is the frailty parameter. 

We also need to generate the family size into the dataset. To this end, we randomly generate family size as one plus the minimum of a fixed number ($n_F-1$) and a value generated from a Poisson random variable with parameter $\lambda_F$. 

\subsection{Semiparametric models}
The first analysis focuses on the semiparametric models, namely, the Multivariate Shared Frailty Cox (MSFCox) model and the Cox model with family history as a fixed covariate (CFHCox) or time-varying covariate (TVFHCox). Notice that the fixed family history indicator takes a value of one if, by the end of each woman's follow-up time, she had at least one female family member who had been diagnosed with breast cancer. Conversely, the time-varying family history indicator $FH(t)$ takes a value of one at the first time when a female family member has been diagnosed with breast cancer, and also the time of the indicator change is tracked.

We first compare, as a check, the true parameter values set for the data generation process to the estimated values. The inference step is performed through an EM algorithm, that can be carried out using functions from the \texttt{R} package \texttt{frailtyEM} \cite{balan2019frailtyem, balan2020tutorial}. However, this package is time and memory-consuming for large dataset. In contrast to the approach commonly found in the literature, we speed up this process by utilizing the \texttt{cox} function in the \texttt{survival} package. This allows us to efficiently obtain all the necessary quantities within a reasonable time and without excessive memory usage.

Results for the configuration with family size parameters set to $(n_F = 5, \lambda_F = 0.8)$, are obtained from 100 repetitions with 100,000 families, and presented in Table \ref{tab:1.1}. Our experience suggests that a sample size of 100,000 families is sufficient to ensure accurate results with reasonable computational speed. 

The MSFCox model can accurately estimate the value of $\theta$ from the given data, while the CFHCox and TVFHCox model estimate the parameter $\beta_{FH}$, which has little meaning because is not part of the data generating process and we can not make any meaningful comparison. In the UFHCox model, the family history indicator $FH$ is utilized as a substitute for the unknown frailty factor. It is interesting to notice that the family history coefficient in the Cox PH regression is always positive as we can appreciate in Table \ref{tab:1.1}, so this means that the family history is still a valid indicator for breast cancer development, due to its association with the family-specific frailty. Therefore, recalling that the indicator $FH$ depends on the family size and on members' birth, these models solely allow estimation of the family history parameter to assess its impact on elucidating the magnitude and timing of breast cancer development but in a weaker way - given all of its dependencies.  The conclusion here is that we can only assess the good recovery of the parameters by the MSFCOx model, and nothing we can say about the other two univariate models. 

The comparison among the models can be made in terms of the concordance index, which compare the estimated results to the true (known) frailties. In the case of the univariate model, the estimated value of the family history indicator is compared to the true risk frailty. Notice also, the value of the Harrell's concordance index also depends on the discrete nature of the predicted risk: for example, if a model predicts only two levels of risk, then the numerator of the index will tend to be small, as all pairs $(i,j)$ for which $\widehat r_i = \widehat r_j$ are excluded. This may happen with the models that include $FH$ and $FH(t)$. 

The result from the previous analysis is that the MSFCox model is, not surprisingly, the best among the considered models, as it is used to generate the data. Given its benchmark concordance index value of about 0.95, the univariate models have a poor concordance performance around only the 50\% of pairs $(i,j)$ to be concordant in terms of time-to-event and risk values. In Table \ref{tab:1.1} the best result by box is in bold and the absolute best by category is highlighted (in this case the best concordance about all the scenarios is highlighted).  
...
\toprule
    ...
    Multivariate &0.2 &0.199 (0.004) &- &\hl{\textbf{0.966 (<0.001)}} \\
    Univariate FH &&- &2.835 (0.081) &0.542 (0.002) \\ 
    Univariate FH(t) &&- &5.846 (0.166) &0.556 (0.002) \\ \midrule
    Multivariate &0.5 &0.499 (0.012) &- &\textbf{0.949 (0.001)} \\ 
    Univariate FH &&- &1.455 (0.040) &0.514 (0.001) \\
    Univariate FH(t) &&- &2.956 (0.083) &0.533 (0.001)  \\ \midrule
    Multivariate &0.8 &0.802 (0.029) &- &\textbf{0.943 (0.001)} \\ 
    Univariate FH &&- &1.102 (0.033) &0.503 (0.001) \\
    Univariate FH(t) &&- &2.227 (0.066) &0.523 (0.001) \\ \bottomrule
    \end{tabular*}
    \caption{mean and standard error of the estimated parameters and concordance index in the last column (C), from the analysis on the semiparametric Cox models.}
    \label{tab:1.1}
\end{table*}
\end{center}
% codice simulation-cox-contfrailty-multiFhCostantAndTimevar-MVV.R LOCAL COMPUTER
Our focus now shifts towards the core of the study, which is the frailty prediction. In this regard, the algorithm developed in Section \ref{sec4} is employed to estimate the risk of developing breast cancer for the subjects in the current dataset and to predict that risk for a new subject who is not part of the initial dataset. The frailty mean and median are predicted for each family, and a comparison is made between the predicted values and the true prior family frailty generated during the initial data generation step. 

The procedure for estimating the frailty parameter and related quantities, including the prediction accuracy, is carried out on the same dataset, without distinguishing between training and validation dataset. We compare the mean and median posterior frailty values in terms of mean square prediction error (MSPE), the coefficient of determination $R^2$ from the regression line of the true frailty on the predicted frailty across all families, the coefficient $\widehat{\beta}$ from the same regression model, and both the Pearson's and the rank correlation coefficient. Results are presented in Table \ref{tab:1.2} for one simulation with 100,000 families and varying family sizes among the values of the family size parameters to be $(n_{F}, \lambda_{F}) = ((2,0.8), (5,0.8), (10,5), (20,10))^T$, as differences emerge when family size increases. The analysis is repeated 100 times to collect mean and standard error of the estimated measures. The best result by row is reported in bold, while the best result by category among models is highlighted. Notice that, the best results overall is when the family parameters are at their maximum value, apart from the estimated mean frailty parameter and the mean square prediction error of the mean frailty, meaning that the family size affects in a positive way the risk prediction accuracy, as expected. Moreover, what we can report from the results of the multivariate setting is that the median posterior frailty performs poorly as a family-specific frailty predictor, especially when family size is small. The posterior mean frailty performs better than the posterior median frailty. Therefore, we adopt the mean as the best posterior frailty index and perform final computations using only the multivariate model, which is up to this point the best in performances. 

However notice that, as the family size decreases, the mean is known to exhibit a shrinkage problem due to the low number of events occurring within families. Figure \ref{fig:scatterquantile} illustrates this phenomenon, in which the posterior mean values do not reach zero when the true frailty value is close to zero, and all values are shrunken towards the frailty mean of one - recall also that we have as prior a Gamma($\theta,\theta$) distribution with mean exactly one -. Notice that the second plot of Figure \ref{fig:scatterquantile} with higher parameters for family size (maximum twenty members per family) has a lower shrinkage compared to the first plot with, indeed, lower family size (maximum five members per family). This well-known phenomenon is due to the fact that a large family size is required to increase the sum of cumulative hazards on observed time and allow the mean to shrink towards zero \cite{balan2019frailtyem, balan2020tutorial}. One should keep in mind this limitation of the posterior frailty mean to predict the family-specific frailty. For large dataset, the shrinkage effect becomes smaller and smaller towards zero and thus we do not encounter this problem for example later on the available data thanks to their high dimension.

\begin{center}
\begin{table*}[!h]%
\begin{tabular*}{\textwidth}{@{\extracolsep\fill}lllll@{}}
\toprule
    % \multicolumn{6}{l}{10000 families} \\ \hline
    %&Univariate 
    ...
    $\widehat\beta_{FH}$ &\textbf{0.91(0.0062)} &0.87(0.0049) &0.62(0.0017) &0.47(0.001) \\ 
...
%     %&Univariate 
%     &\multicolumn{5}{c}{Multivariate model} \\ 
%     %&(1, 0.8) 
%     &(2, 0.8) &(5, 0.8) &(10, 5) &(20, 10) &(50, 10) \\ \hline
%     MSPE(mean) %&\textbf{5.001} 
%     &\textbf{3.788} &\textbf{3.548} &\textbf{2.887} &\textbf{3.496} &\textbf{3.505}   \\ 
%     MSPE(adj. mean) %&5.457 
%     &4.076 &3.749 &2.972 &3.522 &3.531 \\ 
%     MSPE(median) %&9$\cdot 10^{28}$ 
%     &5.809 &5.682 &4.906 &3.973 &3.938 \\ 
%     $R^2$(mean) %&0 
%     &0.227 &0.248 &0.51 &0.649 &0.648 \\ 
%     $R^2$(adj. mean) %&\textbf{0.188} 
%     &\textbf{0.254} &\textbf{0.278} &\textbf{0.514} &\textbf{0.653} &\textbf{0.651}   \\ 
%     $R^2$(median) %&0 
%     &0.166 &0.158 &0.25 &0.291 &0.289   \\ 
%     $\widehat{\beta}_{\text{mean}}$ %&NA 
%     &0.947 &\textbf{0.924} &\textbf{0.719} &\textbf{0.606} &\textbf{0.604}  \\ 
%     $\widehat{\beta}_{\text{adj. mean}}$ %&\textbf{4.021} 
%     &0.823 &0.809 &0.679 &0.589 &0.587 \\ 
%     $\widehat{\beta}_{\text{median}}$ %&NA 
%     &11.664 &9.003 &3.013 &1.445 &1.441  \\ 
%     $\rho$(mean) %&0.401 
%     &0.496 &0.540 &0.736 &0.817 &0.816 \\ 
%     $\rho$(adj. mean) %&\textbf{0.434} 
%     &\textbf{0.520} &0\textbf{.562} &\textbf{0.739} &\textbf{0.819} &\textbf{0.818}   \\ 
%     $\rho$(median) %&0.364 
%     &0.423 &0.423 &0.518 &0.549 &0.547 \\ 
%     Rank $\rho$(mean) %&0.172 
%     &0.242 &0.283 &0.476 &0.5793&0.579 \\ 
%     Rank $\rho$(adj. mean) %&\textbf{0.336} 
%     &\textbf{0.404} &\textbf{0.437} &\textbf{0.605} &\textbf{0.695} &\textbf{0.695}   \\
%     Rank $\rho$(median) %&0.061 
%     &0.113 &0.139 &0.362 &0.493 &0.497 \\ \hline 
%     \end{tabular}
%     \caption{Several prediction accuracy measures, for different family sizes* defined by the two parameters (x, y): x indicates the largest number of family members, and y represents the value of the Poisson distribution used to generate the additional number of family members beyond the main subject. Family size is varying among the couples of maximum and expected value of the minimum: (2, 0.8), (5, 0.8), (10, 5), (20, 10), (50, 10). Notice that ``adj. mean'' stays for adjusted mean.}
%     \label{tab:3}
% \end{table}
% Code from postPrediction3.R on SWEDISH COMPUTER. 
\newpage
...
% \begin{minipage}{0.50\linewidth}
    \centering
    % \includegraphics[width = \linewidth]{plots/Rplot41.pdf}
    \includegraphics[width = \linewidth]{plots/Rplot43.pdf}
    % \includegraphics[width = 0.8\linewidth]{plots/Rplot45.pdf}
    \includegraphics[width = \linewidth]{plots/Rplot47.pdf}
    % \includegraphics[width = \linewidth]{plots/Rplot49.pdf}
    \caption{scatter-plot on 100,000 families of true frailty value versus predicted mean frailty with the bisector of the third quadrant (in red), the smoothed regression line (in green with confidence interval) and smoothed quantile regression line on the first, second and third quantile (in orange) for a smaller and higher family size.}
    \label{fig:scatterquantile}
% \end{minipage}%
% \begin{minipage}{0.50\linewidth}
%     \centering
%     \includegraphics[width = 0.8\linewidth]{plots/Rplot88.jpg}
%     \includegraphics[width = 0.8\linewidth]{plots/Rplot94.jpg}
%     \includegraphics[width = 0.8\linewidth]{}
%     \includegraphics[width = 0.8\linewidth]{}
%     \includegraphics[width = 0.8\linewidth]{}
%     \caption{Scatter-plot on one million families of true frailty value versus predicted mean frailty with the positive bisector, the smoothed regression line (in green with confidence interval) and smoothed quantile regression line on the first, second and third quantile (in orange).}
%     \label{fig:scatterquantile}
% \end{minipage}
\end{figure}
...
Notice in Figure \ref{fig:scatterquantile} that the horizontal bands that can be observed for the smaller family size are due to the form of the posterior frailty mean, which has a discrete component related to the number of events observed in the family. 

The use of the mean as the family-specific predicted frailty highlights another issue, which is the variability in the prediction. The mean squared prediction error (MSPE) is quite high to provide an accurate risk of breast cancer occurrence in a family even when one considers its square root version to recover the correct unit of $\theta$. We explore the values of the root MSPE conditional on the value of the true frailty in Figure \ref{fig:cspe}, but we find no significant differences in the magnitude of the MSPE, which increases as the true frailty increases, regardless of the family size. We observe a possible flattening of the curve with larger family sizes, as shown in the last picture in Figure \ref{fig:cspe}, but the prediction error remained quite high according to the growth of the true frailty value.
\begin{figure}
% \begin{minipage}{0.5\linewidth}
    \centering
    % \includegraphics[width = \linewidth]{plots/Rplot42.pdf}
    \includegraphics[width = \linewidth]{plots/Rplot44.pdf}
    % \includegraphics[width = 0.8\linewidth]{plots/Rplot46.pdf}
    \includegraphics[width = \linewidth]{plots/Rplot48.pdf}
    % \includegraphics[width = \linewidth]{plots/Rplot50.pdf}
    \caption{the conditional MSPE in squared root (y-axis) is plotted against the value of the true frailty (x-axis) for 10000 families. Family size is varying with the values $(n_F, \lambda_F)$: (5, 0.8), (20, 10), with a maximum number of family members up to 5 or 20, respectively.}
    \label{fig:cspe}
% \end{minipage}%
% \begin{minipage}{0.5\linewidth}
%     \centering
%     \includegraphics[width = 0.8\linewidth]{plots/Rplot87.pdf}
%     \includegraphics[width = 0.8\linewidth]{plots/Rplot93.pdf}
%     \includegraphics[width = 0.8\linewidth]{}
%     \includegraphics[width = 0.8\linewidth]{}
%     \includegraphics[width = 0.8\linewidth]{}
%     \caption{The squared conditional MSPE (y-axis) is plotted against the value of the predicted mean frailty (x-axis) for one million families. Family size is varying among the couples of maximum and expected value of the minimum: (2, 0.8), (5, 0.8), (10, 5), (20, 10), (50, 10).}
%     \label{fig:cspe}
% \end{minipage}
\end{figure}
...
Figure \ref{fig:1.3} displays a scatter-plot that depicts the relationship between the true frailty percentiles into the population (prior) distribution and the posterior probability of belonging to the highest-risk families, with the threshold set at 95$\%$ (which of course can be easily changed). From this plot we can appreciate how increasing the family size in the sample the highest-risk families can be more identified by noting a more distinguished point cloud, and a more inclined smoothed line indicating the tendency of the entire cloud. It is interesting to notice that we can identify two risk groups. 
\begin{figure}[ht]
    \centering
...
\begin{table}[ht]
    ...
    &Survival information mother &$\widehat\theta$ &$\widehat\beta_{FH}$ &C \\
...
\begin{table}[ht]
    ...
    &Survival info mother &$\widehat p$ &$\widehat{shape}_0$ &$\widehat{scale}_0$ &$\widehat\theta$/$\widehat\beta_{FH}$ &C \\
...
\begin{table}
    \centering
    \begin{tabular}{l|cccccc} \hline
    Multivariate &$\widehat p_0$ &$\widehat\mu_0 / \widehat{shape}_0$ &$\widehat\sigma^2_0 / \widehat{scale}_0$ &$\widehat\gamma_0$ &$\widehat\theta$/$\widehat\beta_{FH}$ &C \\
