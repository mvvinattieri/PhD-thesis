...
In Table \ref{tab:1.9} we compare the posterior frailty risk mean, median, and mode by reporting their performances in terms of MSPE, $R^2$, coefficient of the predicted risk on the true risk, correlation coefficient, and rank correlation coefficient. Family size is varying across the parameter values $(n_f,\lambda_F)=((2,0.8),(5,0.8),(10,5),(20,10)^T$. In the multivariate and univariate frailty scenario the predicted frailty is identified with the mean, the median, or the mode from the posterior frailty distribution. While in the univariate model with $FH$ the ``estimated frailty'' is the family history indicator itself. In bold are the best results by row, and in yellow are the best results by category across all the models. 
