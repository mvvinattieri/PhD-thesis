...
\subsection{Univariate frailty Cure-Rate models}
...
We move now from the univariate setting to the multivariate setting by computing the multivariate likelihood after a description of the data which enters into the model.
\newline
...
Let us now finally extend the Univariate frailty Lehmann Cure-Rate model to the Multivariate frailty Lehmann Cure-Rate model with two-latent-classes. The assumptions we rely on to deal with the multivariate aspect of this model are the conditional independence among survival times within the same family and the assumption of shared frailty risk within a family which allows the time-to-events to be i.i.d.. 

In this model, the observed data likelihood function incorporates common family memberships by grouping their contributions to the likelihood within each risk group. A deepening about the observed data likelihood is in Appendix \ref{appendix:2.e}. Indeed, let $\underline\theta = \{p_0, \underline\lambda^T, \alpha, h_0\}^T$ be the whole parameter vector of the model, where all the parameters have been defined previously but $\underline\lambda^T,$ which is the collection of baseline survival parameters (whose dimension changes according to the assumed distribution). The observed data likelihood is given by \begin{align*}
